...
module.exports =function Validator(swagger) {
...
function validateType(name, property, field, models, allowBlankTargets, disallowExtraProperties) {
    ...
    if(expectedType === 'array') {
        if(!Array.isArray(property)) {
            return new Error({property: name, error: name + ' is not an array. An array is expected.'});
        }
    ...
    //if(!format) {
        return new Error({property: name, error: name + ' (' + wrapNullProperty(property) + ') is not a type of ' + (format || expectedType)});
    //} else {
...
function validateMinLength(name, value, minLength) {
    if(minLength > 0) {
        if(value.length < minLength) {
            if(minLength === 1) {
                return new Error({property: name, error: name + ' cannot be blank'});
            }
            return new Error({property: name, error:name + ' must be at least ' + minLength.toString() + ' characters long'});
        }
    }
    return null;
}
...
function validateMaxLength(name, value, maxLength) {
    if(maxLength > 0) {
        if(value.length > maxLength) {
            return new Error({property: name, error:name + ' must be no more than ' + maxLength.toString() + ' characters long'});
        }
    }
    return null;
}
...
function validateMinMaxLength(name, value, minLength, maxLength) {
    ...
    if(value.length < minLength || value.length > maxLength){
        if(minLength === 1) {
            return new Error({property: name, error:name + ' cannot be blank and cannot be longer than ' + maxLength.toString() + ' characters long'});
        }
        return new Error({property: name, error:name + ' must be at least ' + minLength.toString() + ' characters long and no more than ' + maxLength.toString() + ' characters long'});
    }
    return null;
}
...
function validateMinValue(name, value, minValue) {
    if(minValue || minValue === 0) {
        if(value < minValue) {
            return new Error({property: name, error:name + ' must be at least ' + minValue.toString()});
        }
    }
    return null;
}
...
function validateMaxValue(name, value, maxValue) {
    if(maxValue || maxValue === 0) {
        if(value > maxValue) {
            return new Error({property: name, error:name + ' must be no more than ' + maxValue.toString()});
        }
    }
    return null;
}
...
function validateMinMaxValue(name, value, minValue, maxValue, exclusive) {
    ...
    if(value < minValue || value > maxValue) {
        return new Error({property: name, error:name + ' must be at least ' + minValue.toString() + ' and no more than ' + maxValue.toString()});
    }
    return null;
}
    ...
    for (var i = 0; i < fields.length; ++i) {
        ...
        try {
            if(!object.hasOwnProperty(property) || object[property] === "" || (object[property] === null && !modelFields[property]['x-nullable'])) {
                errors.push(new Error({property:property, error: property + ' is a required field'}));
            }
        } catch (e) {
...
function validateEnums(name, value, enums) {
    ...
    return new Error({property: name, error:name + ' is not set to an allowed value (see enum)'});
}
